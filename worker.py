...
def loop_worker():
    with app.app_context():
        while True:
            ...
            if agora.minute % 4 == 0 and not ja_executou_hoje(f"distribuir_rendimentos_{agora.hour}_{agora.minute}"):
                print(f"[{agora}] 🚀 Executando distribuição de rendimentos...")
                try:
                    distribuir_rendimentos()
                    registrar_execucao(f"distribuir_rendimentos_{agora.hour}_{agora.minute}")
...
if __name__ == "__main__":
    print("🕒 Worker iniciado. Testando execução a cada 4 minutos...")
    loop_worker()
